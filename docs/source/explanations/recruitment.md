...
The `participant_id` needs to comply with the {term}`BIDS specification <BIDS>`, which only allows alphanumeric characters (i.e. letter and numbers). If any of the sets of participant IDs generated within the study meets this condition, then that can become the designated `participant_id` column. Otherwise, users need to create a new set that is BIDS-compliant.
