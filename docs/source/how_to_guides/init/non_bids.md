...
```console
$ nipoppy init --study <PATH_TO_NEW_DATASET>
```
...
```{attention}
It is extremely unlikely that the example {term}`manifest <manifest file>` accurately represents your dataset, so you will have to generate one yourself. See [this guide](../manifest/index) for more details.
