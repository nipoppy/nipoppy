...
- Python API: {class}`nipoppy.workflows.reorg.DicomReorgWorkflow`
...
See the API reference for {class}`nipoppy.workflows.reorg.DicomReorgWorkflow` for more information on optional arguments (they correspond to the ones for the [CLI](<project:../../cli_reference/reorg.rst>)).
...
The {class}`nipoppy.workflows.reorg.DicomReorgWorkflow` class exposes two functions for finer control of input paths and output filenames:

- {func}`nipoppy.workflows.reorg.DicomReorgWorkflow.get_fpaths_to_reorg` can be overridden to map a participant ID and session ID to a list of absolute filepaths to be reorganized
- {func}`nipoppy.workflows.reorg.DicomReorgWorkflow.apply_fname_mapping` can be overridden to rename output files
