...
def move_target_to_symlink(symlink: Path) -> None:
    '''
    Convert a symlink into its target file by moving the target to the linked location.

    This is for removing unnecessary complexity from completed TractoFlow outputs and
    facilitating the archiving and moving of results.
    '''
    # if the symlink is a link
    if symlink.is_symlink():

        # extract the target path from the link
        target = symlink.resolve()

        # remove the link
        symlink.unlink()

        # rename (move) the target of the link to replace the link
        target.rename(symlink)

    # otherwise, error and exit
    else:
        raise ValueError(f"symlink is not a symlink: {symlink}\nDoing nothing.")
